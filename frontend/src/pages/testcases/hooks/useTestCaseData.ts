...
import { TestCase, Module, Environment, CaseKind } from '../types';
...
export default function useTestCaseData(
  ...
  const fetchData = useCallback(() => {
    ...
    setLoading(true);
    const kindParam = `case_kind=${encodeURIComponent(caseKind)}`;
    if (submittedQuery) {
      axios.get(`/api/testcases/search?project_id=${selectedProject}&q=${encodeURIComponent(submittedQuery)}&page=${page}&size=${pageSize}&${kindParam}`)
    ...
    const url = selectedModuleId
      ? `/api/testcases/module/${selectedModuleId}/testcases?page=${page}&size=${pageSize}&${kindParam}`
      : `/api/testcases/project/${selectedProject}/testcases?page=${page}&size=${pageSize}&${kindParam}`;
    axios.get(url)
      .then((res) => { setData(res.data.items || []); setTotal(res.data.total_items || 0); })
      .catch(logger.error)
      .finally(() => setLoading(false));
  }, [selectedProject, selectedModuleId, page, pageSize, submittedQuery, caseKind]);
