...
const TestCases: React.FC = () => {
  ...
  const openEdit = (tc: TestCase) => { setEditingCase(tc); form.setFieldsValue({ name: tc.name, description: tc.description, module_id: tc.module_id }); setSteps(tc.steps && tc.steps.length ? tc.steps.map((s, i) => ({ step_order: i + 1, description: s.description, parsed_result: s.parsed_result || '', retry_max: s.retry_max ?? 0, retry_delay: s.retry_delay ?? 1.0, healed_selector: s.healed_selector, learned_locator: s.learned_locator ?? null })) : [{ step_order: 1, description: '', parsed_result: '', retry_max: 0, retry_delay: 1.0 }]); setVisible(true); };
  const handleSubmit = async () => { try { const values = await form.validate(); const payload = { project_id: selectedProject, module_id: values.module_id || null, name: values.name, description: values.description, steps: steps.filter((s) => s.description.trim()).map((s) => ({ step_order: s.step_order, description: s.description, parsed_result: s.parsed_result || '', retry_max: s.retry_max ?? 0, retry_delay: s.retry_delay ?? 1.0, learned_locator: s.learned_locator ?? null, healed_selector: s.healed_selector })) }; if (editingCase) { await axios.put(`/api/testcases/${editingCase.id}`, payload); Message.success(t['update.success']); } else { await axios.post('/api/testcases/', payload); Message.success(t['create.success']); } setVisible(false); fetchData(); } catch (e: unknown) { const err = e as { response?: { data?: { detail?: string } } }; if (err.response?.data?.detail) Message.error(err.response.data.detail); } };
