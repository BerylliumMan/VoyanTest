...
def _emit_close_overlays() -> list[str]:
    """Close visible dialogs/notifications via role-based close controls.

    Framework-agnostic: scope to the dialog container, then click the
    accessible close control. Real clicks only; no target is a failure.
    """
    return [
        "    # Close visible dialogs / notifications; no target is a real failure",
        "    closed_count = 0",
        "    _close_name = re.compile(r'^(关闭|Close|X|×)$', re.I)",
        "    for _ in range(5):",
        "        boxes = page.locator(",
        "            '[role=\"dialog\"]:visible, [role=\"alertdialog\"]:visible,'",
        "            ' .el-dialog__wrapper:visible, .ant-modal-root:visible,'",
        "            ' .el-message-box__wrapper:visible'",
        "        )",
        "        if await boxes.count() == 0:",
        "            break",
        "        box = boxes.first",
        "        btn = box.get_by_role('button', name=_close_name)",
        ...
        "            )",
        "        if await btn.count() == 0:",
        "            raise RuntimeError('visible dialog has no close button')",
        "        try:",
        "            await btn.first.click(timeout=3000)",
        "            closed_count += 1",
        "        except Exception as exc:",
        "            raise RuntimeError(f'close dialog failed: {exc}') from exc",
        "    for _sel in (",
        "        '.el-notification:visible .el-notification__closeBtn',",
        "        '.ant-notification-notice-close',",
        "        '[role=\"status\"] [aria-label=\"Close\"]',",
        "        '[role=\"alert\"] [aria-label=\"Close\"]',",
        "        '[role=\"alert\"] [aria-label=\"关闭\"]',",
        "    ):",
        "        for _ in range(5):",
        "            nbtn = page.locator(_sel)",
        "            if await nbtn.count() == 0:",
        "                break",
        "            try:",
        "                await nbtn.first.click(timeout=2000)",
        "                closed_count += 1",
        "            except Exception as exc:",
        "                raise RuntimeError(",
        "                    f'close notification failed: {exc}'",
        "                ) from exc",
        "    if closed_count == 0:",
        "        raise RuntimeError('no visible dialog or notification to close')",
        "",
    ]
