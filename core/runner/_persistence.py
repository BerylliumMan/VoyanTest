...
从 HTTP handler 收到结果后直接落库），因此这里保留 `SessionLocal()`
...
def mark_run_running(db, run_id: int) -> bool:
...
def update_run_on_completion(
    ...
    try:
        ...
        db.execute(stmt)
        append_run_logs(db, run_id, run_log_entries)
        db.commit()
        if batch_id:
            try:
                crud.update_batch_counters(db, batch_id, status)
...
def append_run_logs(db, run_id: int, log_entries: list[dict]) -> None:
