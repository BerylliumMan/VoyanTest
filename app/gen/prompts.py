...
- **禁止**自行编写通用断言，例如臆造「页加载完成」「输入框显示 xxx」「跳转成功」等文档未出现的表述。
- 若某步文档没有给出可观察结果：该步 expected 填「文档未写明预期」，不要编造。
- steps 与 expected **长度仍须相等**；有文档断言的步骤原样摘录（可用【】标关键文案），无则用「文档未写明预期」。
...
[
  {
    ...
    "steps": ["打开【…】页面", "在【…】输入 …", "点击【…】"],
    "expected": ["文档未写明预期", "文档未写明预期", "出现文档所述【提交成功】提示"],
    "scenario_type": "文档流程"
  }
]
"""
