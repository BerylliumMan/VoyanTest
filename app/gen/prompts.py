...
【硬禁令 — 面向执行稳定性】
- 【】内**禁止省略号**（`...` / `…` / `。。。`）；截图看不清时写邻近完整 title / 可见全文，禁止 `【4888智能辅助（高检...）】`
  - 坏：`点击【4888智能辅助（高检...）】` → 好：`点击【4888智能辅助（高检一级部署）】`（以页面 title/全文为准）
- **禁止「关闭所有对话框」并步**；必须拆成：`等待弹窗或对话框出现` → `点击【关闭】`（多个弹窗则重复这两步）
  - 坏：`等待页面中间出现对话框，把所有对话框都点击【关闭】`
  - 好：`等待弹窗或对话框出现` + `点击【关闭】`
