...
【可执行风格 — 硬规则】
每一步必须能被自动化引擎直接执行，不合格的步骤会被校验器整条丢弃：
- 禁止观察体：禁止以"观察/查看/检查/注意"开头，禁止"观察…变化/检查一下/确认结果"。
  观察必须改写——能点的写点击，能断的写断言：
  ✗「观察该按钮的文字变化」→ ✓「断言该按钮文字变为【Remove】」
  ✗「观察右上角徽标数量」→ ✓「断言购物车图标徽标数量为【1】」
- 一步一动作：打开/输入/点击/断言各占一步，禁止并步。
- 首步必须是打开/登录/导航（用例独立可执行，不依赖浏览器残留状态）。
- 末步必须是断言（assert_text/assert_visible），用例要有可验证的收尾。
- 风格范本（照此写）：
...
- 【】里的控件名必须原样写入 target_name，禁止改写或缩写。例如手册写【物料】【查询库存】，就不能写成「商品」或「查询」。
- 文档未写清可省略，不可发明新业务步。
