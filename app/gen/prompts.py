...
- **每个测试项至少覆盖三类**：1 条正常流程 + 1 条异常流程 + 1 条边界场景；允许额外用例。
- 每条用例必须填写 scenario_type，且带上 fp_name（与测试项名称一致），便于核对覆盖。

【查询与组合查询 — 必须遵守】
- **单条件查询**测试项：按文档覆盖有效值查询、空条件、无结果、非法/越权（文档有则写）；scenario_type 用正常/异常/边界。
- **组合查询**测试项（name 含「组合查询」或 desc 要求多条件）：必须生成**两两组合**用例（字段 A+B、A+C…），不做全量笛卡尔积；字段数 n 时目标约 min(n×(n-1)/2, 6) 条组合用例。
- 组合用例的 scenario_type 填 **「组合查询」**。
- 组合步骤写清：填写条件 A → 填写条件 B → 点击【查询】→ 列表结果符合两条件交集（或文档所述逻辑）。

【设计要求】
- module 必须与测试项详情中的模块路径一致（「一级」或「一级——二级」），不要自行修改或改用文件名
- title 用自然语言描述场景意图即可（如「正确手机号验证码登录」「验证码错误无法登录」「姓名与状态组合查询」）
- **禁止**在标题末尾加场景标签后缀：如「-正常」「-异常」「-边界」「-组合」「-正常流程」等；场景类型只写在 scenario_type 字段
...
[
  {
    ...
    "scenario_type": "正常流程 | 异常流程 | 边界场景 | 组合查询"
  }
]

单测试项示例（至少 3 条；组合查询项另加组合用例）：
[
  {"fp_name":"手机号验证码登录成功","title":"正确手机号验证码登录","module":"登录注册——验证码登录","priority":"P0","precondition":"已打开登录页且可收验证码","steps":["输入合法手机号","获取并输入正确验证码","点击【登录】"],"expected":["手机号校验通过","验证码校验通过","进入首页"],"scenario_type":"正常流程"},
  {"fp_name":"手机号验证码登录成功","title":"验证码错误无法登录","module":"登录注册——验证码登录","priority":"P0","precondition":"已打开登录页","steps":["输入合法手机号","输入错误验证码","点击【登录】"],"expected":["提示验证码错误","停留在登录页","不进入首页"],"scenario_type":"异常流程"},
  {"fp_name":"手机号验证码登录成功","title":"验证码过期后登录","module":"登录注册——验证码登录","priority":"P1","precondition":"验证码已超过有效期","steps":["输入合法手机号","输入过期验证码","点击【登录】"],"expected":["提示验证码过期或失效","可重新获取验证码","不进入首页"],"scenario_type":"边界场景"},
  {"fp_name":"客户列表组合查询","title":"姓名与状态组合查询","module":"客户管理——客户列表","priority":"P0","precondition":"已打开客户列表且存在匹配与不匹配数据","steps":["在【姓名】输入张三","在【状态】选择【启用】","点击【查询】"],"expected":["列表仅含姓名匹配且状态为启用的记录","不出现仅满足单条件的记录"],"scenario_type":"组合查询"}
]
"""
