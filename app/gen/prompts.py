...
_UI_STEP_CONTRACT = """
【步骤结构化 — StructuredStep（对齐 Midscene Instant Action）— 必须严格遵守】
- **一步 = 一个浏览器动作**。禁止把「打开→填写→点击」写进同一步。
- **steps 必须是对象数组**，禁止再输出纯字符串步骤。每个对象字段：
  - `action`（必填）：goto|click|fill|select|check|uncheck|wait|assert_text|assert_visible|hover|press_key|click_blank|icon_click
  - `target_name`：页面真实可见文案 / accessible name / aria-label / title / tooltip（禁止控件类型词、禁止省略号）
  - `target_role`（推荐）：button|link|textbox|combobox|option|menuitem|checkbox|radio|img|…
  - `value`：fill 的输入值、select 的选项文案、wait/assert 的文案、goto 的页面名、press_key 的键名
  - `disambiguation`（可选）：同名控件消歧说明
  - `icon_hint`（icon_click 必填）：`{位置}的{外观}图标（用途：{功能}）`
  - `frame_hint` / `note`（可选）
- action 用法：
  1) 打开/进入：`{"action":"goto","target_name":"登录"}`
  2) 点击：`{"action":"click","target_name":"登录","target_role":"button"}`
  3) 输入：`{"action":"fill","target_name":"用户名","target_role":"textbox","value":"admin"}`（值放 value，不要放进 target_name）
  4) 选择：优先两步 click(combobox) + select(option)；或一步 `{"action":"select","target_name":"单位","value":"汉东省院"}`
  5) 勾选：`{"action":"check","target_name":"同意协议"}` / uncheck
  6) 等待：`{"action":"wait","target_name":"Products","value":"Products"}`（禁止无文案的「等待页面加载完成」）
  7) 关闭弹窗：先 wait 提示文案，再 `{"action":"click","target_name":"关闭"}`
  8) 断言：`{"action":"assert_text","value":"保存成功"}` / `{"action":"assert_visible","target_name":"首页"}`
  9) 纯图标无 accessible name：`{"action":"icon_click","icon_hint":"页面右上角书本形状图标（用途：打开操作手册）"}`
- **禁止**把控件类型词放进 target_name：按钮、输入框、下拉框、文本框、图标、图片、logo…
- **禁止** target_name / value 含省略号（`...` / `…`）；截图看不清写完整 title。
- **禁止**一步多目标；相近文案必须写全：提交≠确定≠保存。
- 同名控件：用 disambiguation，或先点唯一行标题再点操作按钮。
- 自定义下拉必须先展开再选：click 字段 → select 选项。

【坏例子（禁止）→ 好例子】
- 坏：`"steps":["点击登录按钮"]` → 好：`{"action":"click","target_name":"登录","target_role":"button"}`
- 坏：`target_name":"单位下拉框"` → 好：`"target_name":"单位"`
- 坏：`target_name":"4888智能辅助（高检...）"` → 好：写完整可见全文
- 坏：`{"action":"click","target_name":"图标"}` → 好：有 label 写【帮助】；纯图形用 icon_click + icon_hint
""".strip()
...
- 若文字写「点击此处」「点击图标」但未写控件名：优先用截图框选区域的 **tooltip / 旁注 / 功能名** 写入【】；
  若旁无文字也无 accessible name，desc 写 `点击{位置}的{外观}图标（用途：{功能}）`（例：`点击右上角书本形状图标（用途：打开操作手册）`）；禁止写成 `点击【图标】`。
...
- 手册写「点击图标」或截图只有图标无字时：优先 aria-label/title；都没有则写
  `点击{位置}的{外观}图标（用途：{功能}）`，禁止 steps 出现裸 `点击【图标】`。
...
- **steps 必须是 StructuredStep 对象数组**（见下方步骤合同），禁止纯字符串步骤
- 禁止：`填写表单`、缺 target_name 的模糊操作、value 误写入 target_name、控件类型词、省略号、`等待页面加载完成`、`target_name=图标`
- 下拉：优先两步 click(combobox) → select(option)
- 提交/打开新页后：加 wait 可见文案
- 有弹窗：先 wait 提示，再 click 关闭/确定

【硬禁令 — 面向执行稳定性】
- target_name / value **禁止省略号**（`...` / `…` / `。。。`）
- **禁止「关闭所有对话框」并步**；拆成 wait + click【关闭】
- 禁止在同一步同时「点击+输入」或「分别/依次」多个主操作
...
[
  {
    ...
    "steps": [
      {"action": "goto", "target_name": "登录"},
      {"action": "wait", "target_name": "Login", "value": "Login"},
      {"action": "fill", "target_name": "用户名", "target_role": "textbox", "value": "admin"},
      {"action": "fill", "target_name": "密码", "target_role": "textbox", "value": "Admin@123"},
      {"action": "click", "target_name": "登录", "target_role": "button"},
      {"action": "wait", "target_name": "首页", "value": "首页"}
    ],
