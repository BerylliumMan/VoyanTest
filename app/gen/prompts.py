...
- **禁止图标空壳步骤**：`点击【图标】` / `点击图标` / `点击【图片】` 一律禁止——执行端无法按「图标」二字定位。
- 纯图标按钮必须用下列之一写入【】（按优先级）：
  1) 悬停/旁注可见的 tooltip、title、aria-label（如「帮助」「手册」「返回」）
  2) 截图红框旁的说明文字或相邻标签（如「操作手册」「书籍」）
  3) 文档对该图标的功能称呼（如「打开帮助文档」→ `点击【帮助】` / `点击【操作手册】`）
- 坏：`点击【图标】` / `点击书本图标按钮`（类型词进【】或主语模糊）
- 好：`点击【操作手册】` / `点击【帮助】` / `点击标题栏带「书本」提示的【操作手册】图标按钮`（【】内仍是可匹配文案「操作手册」）
