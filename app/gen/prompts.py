...
3. 只有确实无法归属的全局能力才使用 module="通用"。
4. 信息不足时可合理推断，但不要遗漏文档已写明的规则与异常路径。
5. **禁止**把上传文件名、图片文件名、`文件N` / `===== … =====` 分隔标记当作 module；分隔条不是业务模块。
...
单测试项示例（必须输出 3 条）：
[
  {"fp_name":"手机号验证码登录成功","title":"正确手机号验证码登录-正常","module":"登录注册","priority":"P0","precondition":"已打开登录页且可收验证码","steps":["输入合法手机号","获取并输入正确验证码","点击【登录】"],"expected":["手机号校验通过","验证码校验通过","进入首页"],"scenario_type":"正常流程"},
  {"fp_name":"手机号验证码登录成功","title":"验证码错误无法登录-异常","module":"登录注册","priority":"P0","precondition":"已打开登录页","steps":["输入合法手机号","输入错误验证码","点击【登录】"],"expected":["提示验证码错误","停留在登录页","不进入首页"],"scenario_type":"异常流程"},
  {"fp_name":"手机号验证码登录成功","title":"验证码过期后登录-边界","module":"登录注册","priority":"P1","precondition":"验证码已超过有效期","steps":["输入合法手机号","输入过期验证码","点击【登录】"],"expected":["提示验证码过期或失效","可重新获取验证码","不进入首页"],"scenario_type":"边界场景"}
]
"""
