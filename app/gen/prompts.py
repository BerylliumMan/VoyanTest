...
- 步骤句式只能用下列模板（【】内为页面真实可见文案）：
  ...
  4) 选择：`在【字段标签】中选择【选项文案】`，或拆成 `点击【字段标签】` → `选择【选项文案】`
...
4. 手册中的等待/提示/关闭弹窗也要写进 desc（如：等待【提交成功】→ 点击【关闭】）。
5. category 可用「操作流程」或「其他」。
6. priority：核心主路径 P0，次要 P1/P2。
7. **禁止**把上传文件名、图片文件名、分隔标记当作 module。
...
输出格式：
{"functional_points":[{"module":"一级——二级","name":"流程名","category":"操作流程","desc":"打开【…】→ 等待页面加载完成 → 点击【…】→ 在【…】输入 … → 点击【…】","priority":"P0|P1|P2"}]}
"""

TC_GENERATE_FLOW_PROMPT = """你是资深的 UI 自动化测试工程师。请根据以下**操作流程摘要**，并结合用户消息中附带的**手册原文与截图**（若有），生成**浏览器可执行的 UI 自动化用例**。
生成结果将交给 Playwright/AI 执行：漏步骤、并步骤、控件名臆造都会导致定位失败。
...
- 仅为稳定性插入的「等待页面加载完成」步骤：expected 对应填 `""`。
...
[
  {
    ...
    "steps": ["打开【…】页面", "等待页面加载完成", "点击【…】", "在【…】输入 …", "点击【提交】", "等待【提交成功】出现", "点击【关闭】"],
