...
- **每个测试项至少覆盖三类**：1 条主路径成功 + 1 条异常/校验提示 + 1 条边界或状态类；允许额外用例。
- 每条用例必须填写 scenario_type，且带上 fp_name（与测试项名称一致）。

【设计要求 — UI 自动化专用】
- module 必须与测试项详情中的模块路径一致（「一级」或「一级——二级」）
- title 含页面/操作意图，用自然语言描述；**禁止**标题末尾加「-正常」「-异常」「-边界」等后缀（场景类型只放 scenario_type）
- precondition 写清起始页面或登录态（如：已打开登录页 / 已登录普通用户）
- **steps 与 expected 数组长度必须完全相等**
- expected：中间操作步默认 `""`；仅关键结果步写可观察断言（文案、按钮、URL、元素出现/消失）；禁止空编号 `1、`/`2、`
