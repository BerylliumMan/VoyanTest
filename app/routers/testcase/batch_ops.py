...
def batch_move_cases(req: BatchMoveRequest, admin=Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    ...
    for case_id in req.case_ids:
        case = crud.get_test_case(db, case_id)
        if not case:
            continue
        ...
        if is_cross_project:
            case.project_case_number = get_next_project_case_number(db, req.project_id)
        moved += 1
    db.commit()
...
def batch_copy_cases(req: BatchCopyRequest, admin=Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    ...
    copied = 0
    for case_id in req.case_ids:
        original = crud.get_test_case(db, case_id)
        if not original:
            continue
        steps_data = [
        ...
        crud.create_test_case(db, new_case)
        copied += 1
