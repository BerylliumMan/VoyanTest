...
DEFAULT_AGENTS: list[dict] = [
    ...
    },
    {
        ...
        "constraints": [],
        "thinking_config": {},
        "system_prompt": "你是 UI 自动化用例生成专家。流程：(1) fp_extract 提取细粒度测试项；(2) tc_generate_ui 输出浏览器可执行步骤。按类型选型（主路径/校验提示/空态/组合查询），禁止机械凑正常/异常/边界；优先可自动化路径。硬规则：一步一动作；【】标可见文案；禁止控件类型词进【】；等待用「等待【可见文案】出现」；同名须消歧；中间 expected 默认空。",
        "prompt_overrides": {}
    },
    {
        "name": "流程手册用例生成助手",
