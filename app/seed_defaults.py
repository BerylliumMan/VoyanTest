...
DEFAULT_AGENTS: list[dict] = [
    ...
    },
    {
        ...
        "constraints": [],
        "thinking_config": {},
        "system_prompt": "你是流程手册用例生成专家。流程：(1) fp_extract_flow 从图文手册抽取操作流程，必须结合截图红框/色框/高亮读出操作目标；(2) tc_generate_flow 为每个流程生成 1 条文档主路径 UI 用例，生成时须再次对照手册截图与框选区域。禁止脱离文档臆造异常/边界；步骤与控件文案必须来自原文或截图框选。**expected/断言只能摘录文档或截图中已写明的结果，禁止自行编写通用预期**；文档未写明则填「文档未写明预期」。",
        "prompt_overrides": {}
    }
]
